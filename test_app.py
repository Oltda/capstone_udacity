...
class WarehouseTestCase(unittest.TestCase):
    ...
    def test_post_warehouse(self):
        ...
        res = self.client().post('/warehouse', json=self.new_warehouse,
    ...
    def test_405_post_warehouse(self):
        ...
        res = self.client().post('/warehouse/2', json=self.new_warehouse,
                                 headers=get_token(manager_token))
        data = json.loads(res.data)
    ...
    def test_patch_warehouse(self):
        res = self.client().patch('/warehouse/2',
                                  json={
                                      'name': 'edited name',
    ...
    def test_422_patch_warehouse(self):
        res = self.client().patch('/warehouse/554',
                                  json={
                                      'name': 'altered name',
                                      'address':'altered address'
                                  },
                                  headers=get_token(manager_token))
        data = json.loads(res.data)
    ...
    def test_post_code(self):
        res = self.client().post('/product-code', json=self.new_product_code,
                                 headers=get_token(manager_token))
        data = json.loads(res.data)
    ...
    def test_405_post_product_code(self):
        ...
        res = self.client().post('/product-code/2654', json=self.new_product_code,
                                 headers=get_token(manager_token))
        data = json.loads(res.data)
    ...
    def test_post_stock_items(self):
        res = self.client().post('/stock-items', json=self.new_stock_item,
                                 headers=get_token(manager_token))
        data = json.loads(res.data)
    ...
    def test_405_post_stock_items(self):
        ...
        res = self.client().post('/stock-items/2', json=self.new_stock_item,
                                 headers=get_token(manager_token))
        data = json.loads(res.data)
    ...
    def test_delete_stock_item(self):
        stock = StockItems.query.filter(StockItems.id == 2).one_or_none()
        if stock is None:
    ...
    def test_delete_product_code(self):
        code = ProductCodes.query.filter(ProductCodes.id == 2).one_or_none()
        if code is None:
    ...
    def test_RBAC_patch_warehouse_employee(self):
        res = self.client().patch('/warehouse/2', json={
                                    'name': 'employee changes',
                                    'address': 'employee address'},
                                  headers=get_token(employee_token))
        data = json.loads(res.data)
